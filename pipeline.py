...
LUA_SHA1 = get_hash(os.path.join(CWD, 'goodictionary.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            WGET_AT,
            '-U', USER_AGENT + ' ' + ''.join(random.sample(string.digits+string.ascii_lowercase, 16)),
            '-nv',
            '--no-cookies',
            '--host-lookups', 'dns',
            '--hosts-file', '/dev/null',
            ...
            '--header', 'Referer: https://dictionary.goo.ne.jp/'
        ]
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'path':
                wget_args.extend(['--warc-header', 'goodictionary-path: '+item_value])
                wget_args.append('https://dictionary.goo.ne.jp'+item_value)
            else:
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/9/9a/Goo-dictionary-icon.png" height="50px" title=""/>
        <h2>goo辞書 <span class="links"><a href="https://dictionary.goo.ne.jp/">Website</a> &middot; <a href="http://tracker.archiveteam.org/goodictionary/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/goo辞書">Wiki</a></span></h2>
        <p>Archiving goo辞書.</p>
    '''
)
